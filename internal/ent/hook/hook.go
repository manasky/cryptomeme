...
func (f MarketsFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.MarketsMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MarketsMutation", m)
	}
	return f(ctx, mv)
}
